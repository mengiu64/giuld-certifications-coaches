...
const SERVICE_SNIPPETS: Record<string, { domain: string; entries: Array<{ title: string; url: string; snippet: string }> }> = {
  ec2: {
    domain: 'compute',
    entries: [
      {
...
const toResult = (serviceName: string, domain: string, entry: { title: string; url: string; snippet: string }): DocumentationResult => ({
  title: entry.title,
  url: entry.url,
...
export const searchByService = (serviceName: string, topic?: string): DocumentationResult[] => {
