...
server.setRequestHandler(ListToolsRequestSchema as never, async () => ({ tools: [...tools] }));

server.setRequestHandler(CallToolRequestSchema as never, async (request: any) => {
  ...
  switch (toolName) {
    ...
    case 'search_by_topic':
      result = searchByTopic(
        String(args.query ?? ''),
        Array.isArray(args.domains) ? args.domains.map(String) : undefined,
