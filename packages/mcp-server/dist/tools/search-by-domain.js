...
export const searchByDomain = (domain, topic) => {
    ...
    const filtered = results.filter((result) => result.title.toLowerCase().includes(normalizedTopic) ||
        result.snippet.toLowerCase().includes(normalizedTopic) ||
        result.service.toLowerCase().includes(normalizedTopic));
    return filtered.length > 0 ? filtered : results;
};
//# sourceMappingURL=search-by-domain.js.map