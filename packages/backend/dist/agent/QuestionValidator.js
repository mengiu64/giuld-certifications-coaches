...
export class QuestionValidator {
    static normalizeDraft(draft) {
        ...
        const correctAnswers = [...new Set(draft.correctAnswers.map((answer) => answer.trim().toUpperCase()))]
            .filter((answer) => validLabels.has(answer))
            .sort();
    ...
    static validateQuestion(question) {
        ...
        const validCorrectAnswerCounts = {
            'single-4': [1],
            'multi-5': [2, 3],
            'multi-6': [2, 3],
        };
        ...
        if (normalized.correctAnswers.some((answer) => !optionLabelSet.has(answer))) {
    ...
    static assertValidQuestion(question) {
