...
import { BEDROCK_INTER_REQUEST_DELAY_MS, FORMAT_OPTION_COUNT, QUESTION_LABELS, generatedQuestionDraftSchema, } from '@aws-exam-generator/shared';
...
const formatCorrectAnswerCount = (format) => {
    switch (format) {
        case 'single-4':
            return ['A'];
        case 'multi-5':
...
export class QuestionGenerator {
    ...
    mcpClient;
    allowMockFallback;
    lastBedrockRequestAt = 0;
    constructor(bedrockClient, mcpClient, allowMockFallback) {
        this.bedrockClient = bedrockClient;
        this.mcpClient = mcpClient;
        this.allowMockFallback = allowMockFallback;
    }
    async generateQuestion(certification, domainId, format) {
    ...
    async fetchDocumentation(domainId, certification) {
    ...
    buildSystemPrompt(certification, domainId, format, context) {
...
Respond ONLY with valid JSON matching this schema:
{
  ...
  "referenceUrl": "https://docs.aws.amazon.com/..."
    ...
    buildUserPrompt(certification, domainId, format) {
...
Only output JSON.`;
    ...
    buildMockQuestion(certification, domainId, format, docs) {
