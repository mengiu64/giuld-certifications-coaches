...
import { DEFAULT_CERTIFICATION_ID, FORMAT_OPTION_COUNT, GENERATION_CHECKPOINT_INTERVAL, generationStatusSchema, } from '@aws-exam-generator/shared';
...
const distributeCounts = (weights, total) => {
...
export class ExamAgentController {
    ...
    constructor(questionGenerator, questionBankManager, certificationRegistry, checkpointPath) {
    ...
    async startGeneration(certificationId) {
        if (this.active) {
    ...
    async runGeneration(certification, bankId) {
        try {
    ...
    buildPlan(certification) {
        const domainCounts = distributeCounts(certification.domains.map((domain) => ({ key: domain.id, weight: domain.percentage })), certification.totalQuestions);
        ...
        const domainQueue = Array.from(domainCounts.entries()).flatMap(([domainId, count]) => Array.from({ length: count }, () => domainId));
        const formatQueue = Array.from(formatCounts.entries()).flatMap(([format, count]) => Array.from({ length: count }, () => format));
...
//# sourceMappingURL=ExamAgentController.js.map